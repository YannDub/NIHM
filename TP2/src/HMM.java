	...
	*/
	...
	public ArrayList<Double> computeFeatures(Vector<Point2D> points) {
		...
		for(Point2D p : points) {
			if(pred != null) {
				Point2D vector = pred.subtract(p);
				res.add(Math.abs(Math.toDegrees(Math.atan2(vector.getY(), vector.getX())) / 10));
			}
			pred = p;
		}
		return res;
	...
	protected Vector<Point2D> resample(Vector<PointData> pts, int deltaTms) {
		...
		PointData origin = pts.get(0);
		Point2D currentPoint = new Point2D(origin.getX(), origin.getY());
		res.add(currentPoint);
		long currentTimestamp = origin.getTimeStamp();
		
		
		ListIterator<PointData> lists = pts.listIterator();
		PointData pred = lists.next();
		PointData p = lists.next();
		while(lists.hasNext()) {
			while(lists.hasNext() && currentTimestamp + deltaTms > pred.getTimeStamp()) {
				pred = lists.next();
				currentPoint = new Point2D(pred.getX(), pred.getY());
			}
			
			if(lists.hasNext()) {
				p = lists.next();
				long nextTimestamp = p.getTimeStamp();
				double x = (p.getX() - currentPoint.getX()) / (double) ((nextTimestamp - currentTimestamp)) * (double) deltaTms;
				double y = (p.getY() - currentPoint.getY()) / (double) ((nextTimestamp - currentTimestamp)) * (double) deltaTms;
				currentTimestamp += deltaTms;
				currentPoint = new Point2D(currentPoint.getX() + x, currentPoint.getY() + y);
				res.add(currentPoint);
			}
